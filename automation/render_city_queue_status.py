...
if packages_manifest.exists():
    try:
        ...
        package_lines.append(f'- بسته‌های ۵تایی آماده: **{pm.get("ready_batches",0)}**')
    ...
    package_lines.append('- هنوز بسته ۵تایی آماده نشده است.')
lines=[
'# وضعیت زنده صف تولید پست‌های شهری','',
'> این صفحه پس از پردازش هر پست به‌روزرسانی می‌شود. برای دیدن مقدار تازه، صفحه را Refresh کنید.','',
f'- آخرین بروزرسانی: `{now}`',f'- وضعیت صف: **{status.get("result","unknown")}**',
f'- پیشرفت: **{done} از {total} ({percent:.2f}٪)**',f'- تکمیل‌شده: **{done}**',
f'- در حال پردازش: **{status.get("processing",len(processing))}**',f'- در انتظار: **{status.get("pending",0)}**',
f'- ناموفق: **{status.get("failed",len(failed))}**',f'- مسدودشده توسط مدل تصویر: **{status.get("blocked_image_model",0)}**',
f'- مدل متن و بازبینی: `agnes-3.0-flash`',f'- مدل تصویر: `{status.get("image_model","agnes-image-2.0-flash")}`',
f'- فاصله شروع پست بعدی: **۳۰ ثانیه**','',
'## بسته‌های آماده آپلود',''] + package_lines + ['', '## آخرین پست‌های تکمیل‌شده','']
if completed:
 for item in sorted(completed,key=lambda x:x.get('completed_at',''),reverse=True)[:20]:
  topic=item.get('topic','—')
  lines.append(f'- **{item.get("city","—")}** — {item.get("province","—")} — `{topic}` — {item.get("word_count","—")} کلمه — `{item.get("completed_at","—")}`')
...
lines+=['','## فایل‌های خروجی','','- [وضعیت ماشینی](./status.json)','- [صف کامل](./queue.json)','- [SQL تجمیعی انتشار](./create-all-completed.sql)','- [SQL تجمیعی بازگشت](./rollback-all-completed.sql)','- [پوشه بسته‌های ۵تایی](./packages/)','']
