...
def make_content(item,links):
 ...
 for _ in range(4):
  obj=agnes(prompt);body=obj.get('html','');used=internal_links(body);allowed={x['url'] for x in approved}
  if words(body)>=MIN_WORDS and MIN_LINKS<=len(used)<=7 and not(used-allowed) and all(body.count(f'[[[IMAGE_{i}]]]')==1 for i in range(1,4)):return obj
...
def process(q):
 ...
 (OUT/'create-all-completed.sql').write_text('\n'.join(['-- Review before importing. All generated posts are drafts.']+[p.read_text(encoding='utf-8') for p in sorted(SQL.glob('*.sql'))]),encoding='utf-8');(OUT/'rollback-all-completed.sql').write_text('\n'.join(p.read_text(encoding='utf-8') for p in sorted(ROLLBACK.glob('*.sql'))),encoding='utf-8');write_status(q,'ready')
def main():
 import argparse
